...
// input: points(b, c, n) idx(b, m)
// output: out(b, c, m)
__global__ void gathering_int_forward_cuda_kernel(int b, int c, int n, int m, const int *points, const int *idx, int *out)
{
    for (int i = blockIdx.x; i < b; i += gridDim.x)
    {
        for (int l = blockIdx.y; l < c; l += gridDim.y)
...
// input: grad_out(b, c, m) idx(b, m)
// output: grad_points(b, c, n)
__global__ void gathering_int_backward_cuda_kernel(int b, int c, int n, int m, const float *grad_out, const int *idx, float *grad_points)
{
    for (int i = blockIdx.x; i < b; i += gridDim.x)
    {
        for (int l = blockIdx.y; l < c; l += gridDim.y)
...
{
    gathering_int_forward_cuda_kernel<<<dim3(b, c, 1), opt_n_threads(m), 0>>>(b, c, n, m, points, idx, out);
}

void gathering_int_backward_cuda_launcher(int b, int c, int n, int m, const float *grad_out, const int *idx, float *grad_points)
{
    gathering_int_backward_cuda_kernel<<<dim3(b, c, 1), opt_n_threads(m), 0>>>(b, c, n, m, grad_out, idx, grad_points);
}

// ------------------------------ gathering cluster -----------------------------------
// input: points(b, c, n) idx(b, m) idx_3d(b, m, k)
// output: out(b, c, m)
__global__ void gathering_cluster_forward_cuda_kernel(int b, int c, int n, int m, int k, const float *points, const int *idx, const int *idx_3d, float *out)
{
    for (int i = blockIdx.x; i < b; i += gridDim.x)
    {
        for (int l = blockIdx.y; l < c; l += gridDim.y)
...
// output: grad_points(b, c, n)
__global__ void gathering_cluster_backward_cuda_kernel(int b, int c, int n, int m, int k, const float *grad_out, const int *idx, const int *idx_3d, float *grad_points)
{
    for (int i = blockIdx.x; i < b; i += gridDim.x)
    {
        for (int l = blockIdx.y; l < c; l += gridDim.y)
...
{
    gathering_cluster_forward_cuda_kernel<<<dim3(b, c, 1), opt_n_threads(m), 0>>>(b, c, n, m, k, points, idx, idx_3d, out);
}
...
{
    gathering_cluster_backward_cuda_kernel<<<dim3(b, c, 1), opt_n_threads(m), 0>>>(b, c, n, m, k, grad_out, idx, idx_3d, grad_points);
}
