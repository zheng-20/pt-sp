...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, m):
        ...
        pointops_sp_cuda.furthestsampling_cuda(b, n, m, xyz, temp, idx)
        return idx
...
class Gathering(Function):
    ...
    def forward(ctx, features, idx):
        ...
        pointops_sp_cuda.gathering_forward_cuda(b, c, n, m, features, idx, output)
...
class GatheringInt(Function):
    ...
    def forward(ctx, features, idx):
        ...
        pointops_sp_cuda.gathering_int_forward_cuda(b, c, n, m, features, idx, output)
        ctx.for_backwards = (idx, c, n)
        return output
    ...
    def backward(ctx, grad_out):
        ...
        grad_features = torch.cuda.FloatTensor(b, c, n).zero_()
        grad_out_data = grad_out.data.contiguous()
        pointops_sp_cuda.gathering_int_backward_cuda(b, c, n, m, grad_out_data, idx, grad_features.data)
        return grad_features, None
...
class Gathering_Cluster(Function):
    ...
    def forward(ctx, features, idx, idx_3d):
        ...
        k = idx_3d.size(2)
        output = torch.cuda.FloatTensor(b, c, m)
        pointops_sp_cuda.gathering_cluster_forward_cuda(b, c, n, m, k, features, idx, idx_3d, output)
    ...
    def backward(ctx, grad_out):
        ...
        grad_features = torch.cuda.FloatTensor(b, c, n).zero_()
        grad_out_data = grad_out.data.contiguous()
        pointops_sp_cuda.gathering_cluster_backward_cuda(b, c, n, m, k, grad_out_data, idx, idx_3d, grad_features.data)
        return grad_features, None, None
...
class NearestNeighbor(Function):
    ...
    def forward(ctx, unknown: torch.Tensor, known: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        pointops_sp_cuda.nearestneighbor_cuda(b, n, m, unknown, known, dist2, idx)
...
class Interpolation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        ...
        pointops_sp_cuda.interpolation_forward_cuda(b, c, m, n, features, idx, weight, output)
        return output
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        grad_out_data = grad_out.data.contiguous()
        pointops_sp_cuda.interpolation_backward_cuda(b, c, n, m, grad_out_data, idx, weight, grad_features.data)
        return grad_features, None, None
...
class Grouping(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        pointops_sp_cuda.grouping_forward_cuda(b, c, n, m, nsample, features, idx, output)
    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        grad_features = torch.cuda.FloatTensor(b, c, n).zero_()
        grad_out_data = grad_out.data.contiguous()
        pointops_sp_cuda.grouping_backward_cuda(b, c, n, m, nsample, grad_out_data, idx, grad_features.data)
        return grad_features, None
...
class GroupingInt(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        pointops_sp_cuda.grouping_int_forward_cuda(b, c, n, m, nsample, features, idx, output)
        return output
...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        ...
        pointops_sp_cuda.ballquery_cuda(b, n, m, radius, nsample, new_xyz, xyz, idx)
        return idx
...
class FeatureDistribute(Function):
    ...
    def forward(ctx, max_xyz: torch.Tensor, xyz: torch.Tensor) -> torch.Tensor:
        ...
        pointops_sp_cuda.featuredistribute_cuda(b, n, m, max_xyz, xyz, distribute_idx)
...
class FeatureGather(Function):
    ...
    def forward(ctx, max_feature: torch.Tensor, distribute_idx: torch.Tensor) -> torch.Tensor:
        ...
        pointops_sp_cuda.featuregather_forward_cuda(b, n, m, c, max_feature, distribute_idx, distribute_feature)
    ...
    def backward(ctx, grad_distribute_feature: torch.Tensor):
        ...
        pointops_sp_cuda.featuregather_backward_cuda(b, n, m, c, grad_distribute_feature_data, distribute_idx, grad_max_feature.data)
...
class LabelStatBallRange(Function):
    ...
    def forward(ctx, radius: float, xyz: torch.Tensor, new_xyz: torch.Tensor, label_stat: torch.Tensor) -> torch.Tensor:
        ...
        pointops_sp_cuda.labelstat_ballrange_cuda(b, n, m, radius, nclass, new_xyz, xyz, label_stat, new_label_stat)
...
class LabelStatIdx(Function):
    ...
    def forward(ctx, nsample: int, label_stat: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        m = idx.size(1)
        new_label_stat = torch.cuda.IntTensor(b, m, nclass).zero_()
        pointops_sp_cuda.labelstat_idx_cuda(b, n, m, nsample, nclass, label_stat, idx, new_label_stat)
...
class LabelStatAndBallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor, label_stat: torch.Tensor):
        ...
        pointops_sp_cuda.labelstat_and_ballquery_cuda(b, n, m, radius, nsample, nclass, new_xyz, xyz, label_stat, idx, new_label_stat)
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        pointops_sp_cuda.knnquery_cuda(b, n, m, nsample, xyz, new_xyz, idx, dist2)
        return idx
...
class KNNQueryCluster(Function):
    ...
    def forward(ctx, nsample: int, xyz: torch.Tensor, xyz_idx: torch.Tensor, new_xyz: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        dist2 = torch.cuda.FloatTensor(b, m, nsample).zero_()
        pointops_sp_cuda.knnquerycluster_cuda(b, n, m, nsample, xyz, xyz_idx, new_xyz, idx, idx_abs, dist2)
        return idx, idx_abs
...
class KNNQueryClusterGT(Function):
    ...
    def forward(ctx, nsample: int, xyz: torch.Tensor, xyz_idx: torch.Tensor, xyz_gt: torch.Tensor, new_xyz: torch.Tensor = None, new_xyz_gt: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        dist2 = torch.cuda.FloatTensor(b, m, nsample).zero_()
        pointops_sp_cuda.knnqueryclustergt_cuda(b, n, m, nsample, xyz, xyz_idx, xyz_gt, new_xyz, new_xyz_gt, idx, idx_abs, idx_gt, idx_gt_abs, dist2)
...
class KNNQueryPoint(Function):
    ...
    def forward(ctx, ks: int, nsample: int, idx_c: torch.Tensor, cid: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        n = idx_c.size(1)
        idx = torch.cuda.IntTensor(b, m, nsample).zero_()
        pointops_sp_cuda.knnquerypoint_cuda(b, n, m, ks, nsample, idx_c, cid, idx)
        return idx
...
class AssoMatrix(Function):
    ...
    def forward(ctx, ks: int, idx_c: torch.Tensor, cid: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        pointops_sp_cuda.assomatrix_cuda(b, n, m, ks, idx_c, cid, idx, cnt)
        return idx, cnt
...
class AssoMatrixPlusLabel(Function):
    ...
    def forward(ctx, ks: int, idx_c: torch.Tensor, lab: torch.Tensor, cid: torch.Tensor = None, category=13) -> Tuple[torch.Tensor]:
        ...
        pointops_sp_cuda.assomatrix_label_cuda(b, n, m, ks, category, idx_c, lab, cid, idx, cnt, clab)
        return idx, cnt, clab
...
class AssoMatrixFloat(Function):
    ...
    def forward(ctx, ks: int, val_c: torch.Tensor, idx_c: torch.Tensor, cid: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        cnt = torch.cuda.IntTensor(b, m, 1).zero_()
        pointops_sp_cuda.assomatrix_float_cuda(b, n, m, ks, val_c, idx_c, cid, idx, cnt)
        return idx, cnt
...
class AssoFixP2C(Function):
    ...
    def forward(ctx, ks: int, nsample: int, idx_c: torch.Tensor, cid: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        n = idx_c.size(1)
        idx = torch.cuda.IntTensor(b, m, nsample).zero_()
        pointops_sp_cuda.assofixp2c_cuda(b, n, m, ks, nsample, idx_c, cid, idx)
        return idx
...
class AssoFixP2CWeight(Function):
    ...
    def forward(ctx, ks: int, nsample: int, val_c: torch.Tensor, idx_c: torch.Tensor, cid: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        pointops_sp_cuda.assofixp2c_weight_cuda(b, n, m, ks, nsample, val_c, idx_c, cid, idx)
        return idx
