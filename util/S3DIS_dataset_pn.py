...
def read_structure(file_name, read_geof):
    ...
    # 添加法线、基元参数
    normals = np.array(data_file['normals'], dtype='float32')
    T_param = np.array(data_file['T_param'], dtype='float32')
    ...
    data_file.close()
    return xyz, rgb, edg_source, edg_target, is_transition, local_geometry, geof, labels, objects, elevation, xyn, normals, T_param
...
def graph_loader(entry, train, args, logger, db_path, test_seed_offset=0, full_cpu = False):
    ...
    xyz, rgb, edg_source, edg_target, is_transition, local_geometry, geof, \
            labels, objects, elevation, xyn, normals, T_param = read_structure(entry, False)
    # xyz: 点云坐标 n*3
    # rgb: 点云特征（rgb）n*3
    ...
    if train:
        xyz, rgb, normals = augment_cloud_whole(args, xyz, rgb, normals)
...
def my_collate(batch):
    short_name, edg_source, edg_target, is_transition, labels, objects, xyz, normals, T_param = list(zip(*batch))
    ...
    # for i in range(4):
    #     if clouds_global[i].shape[1]!=15000:
    #         import ipdb
    #         ipdb.set_trace()
...
class s3dis_Dataset_pn(Dataset):
    ...
    def __getitem__(self, index):
        ...
        """ Load the point cloud and the graph structure """
        xyz, rgb, edg_source, edg_target, is_transition, local_geometry, geof, \
                labels, objects, elevation, xyn, normals, T_param = read_structure(data, False)
        # xyz: 点云坐标 n*3
        # rgb: 点云特征（rgb）n*3
        # edg_source: 点云5邻域种子点索引 5n
        # edg_target: 点云5邻域邻域点索引 5n
