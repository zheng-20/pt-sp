...
def compute_graph_nn_2(xyz, k_nn1, k_nn2, voronoi = 0.0):
    ...
    distances, neighbors = nn.kneighbors(xyz)   # 查找每个点的k_nn2+1个最近邻，返回的是距离和索引
    del nn
    neighbors = neighbors[:, 1:]    # 去掉自身
    distances = distances[:, 1:]
    #---knn2---
    target2 = (neighbors.flatten()).astype('uint32')    # 每个点的k_nn2个最近邻的索引，按行展开
    #---knn1-----
    if voronoi>0:
        tri = Delaunay(xyz)
        ...
        neighbors = neighbors[:, :k_nn1]    # 只保留k_nn1个最近邻
        distances = distances[:, :k_nn1]
        graph["source"] = np.matlib.repmat(range(0, n_ver)
            , k_nn1, 1).flatten(order='F').astype('uint32') # n*5, 每个点的索引，按行展开
        graph["target"] = np.transpose(neighbors.flatten(order='C')).astype('uint32')   # n*5, 每个点的k_nn1个最近邻的索引，按行展开
        graph["distances"] = distances.flatten().astype('float32')
    #save the graph
