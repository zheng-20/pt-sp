...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, loss_re_xyz, loss_re_label, loss_re_sp, loss_re_norm, loss_norm_consis, loss_re_param, loss_p2sp_contrast, f1_score = train(train_loader, model, criterion, criterion_re_xyz, criterion_re_label, criterion_re_sp, optimizer, epoch, scaler, scheduler)
        if args.scheduler_update == 'epoch':
            scheduler.step()
        epoch_log = epoch + 1
        is_best = False
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            ...
            if scheduler is not None:
                torch.save({'epoch': epoch_log, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict(),
                             'scheduler': scheduler.state_dict(), 'best_f1_score': best_f1_score, 'is_best': is_best}, filename)
            else:
...
def train(train_loader, model, criterion, criterion_re_xyz, criterion_re_label, criterion_re_sp, optimizer, epoch, scaler, scheduler):
    ...
    return loss_meter.avg, loss_re_xyz_meter.avg, loss_re_label_meter.avg, loss_re_sp_meter.avg, loss_re_norm_meter.avg, loss_norm_consis_meter.avg, loss_re_param_meter.avg, loss_p2sp_contrast_meter.avg, f1_score
...
def validate(val_loader, model, criterion, criterion_re_xyz, criterion_re_label, criterion_re_sp):
    ...
    for i, (filename, coord, rgb, normals, label, semantic, param, offset, edg_source, edg_target, is_transition) in enumerate(val_loader):
        data_time.update(time.time() - end)
        # coord, feat, target, offset = coord.cuda(non_blocking=True), feat.cuda(non_blocking=True), target.cuda(non_blocking=True), offset.cuda(non_blocking=True)
        # coord, normals, boundary, label, semantic, param, offset, edges = coord.cuda(non_blocking=True), normals.cuda(non_blocking=True), boundary.cuda(non_blocking=True), \
        #             label.cuda(non_blocking=True), semantic.cuda(non_blocking=True), param.cuda(non_blocking=True), offset.cuda(non_blocking=True), edges.cuda(non_blocking=True)
